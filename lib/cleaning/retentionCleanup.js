...
async function deleteOldRawMessages(retentionDays = 7) {
  ...
  try {
    ...
    // Chunk IN queries to avoid Supabase payload limits
    const allOldIds = oldRaw.map(r => r.message_id);
    const CHUNK_SIZE = 500;
    const idsToDelete = [];
    for (let i = 0; i < allOldIds.length; i += CHUNK_SIZE) {
      const chunk = allOldIds.slice(i, i + CHUNK_SIZE);
      ...
      if (err3) {
        throw new Error(`Failed to verify cleaned messages (chunk ${i / CHUNK_SIZE + 1}): ${err3.message}`);
      }
      
      if (verified) {
        for (const v of verified) {
          idsToDelete.push(v.message_id);
        }
      }
    }
